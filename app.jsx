...
class CommentForm extends React.Component{
   ...
   handleSubmit(e){
      ...
      // console.log(form);
      //拿到了页面上表单的值  就提交给服务器
      this.props.onSubmit({author:author,body:body});//表示调用  调用这个组件的是传入的props方法 也就是Commentbox里的子组件的props  onSubmit方法
      form.reset();//表单重置
   }
...
class CommentList extends React.Component{
   render(){
      ...
      var commentsnode=this.props.comments.map(function(comments,index) {
         return <Comment author={comments.author} key={'comments'+index}>{comments.body}</Comment>
      });
       return (
